...
namespace Backend_webAPI_ASP.NETCore.Controllers
{
    ...
    public class HomeController : Controller
    {
        ...
        */
        [HttpPost]
        public IActionResult EnviarDatos([FromBody] List<Contacto> contactos)
        {
            //-------------- Código nuevo: --------------------------
            // Conexión a SAP B1
            try
            {
                ...
                if (err != 0)
                {
                    _logger.LogError("Error de conexión: " + myCompany.GetLastErrorDescription());
                    return StatusCode(StatusCodes.Status500InternalServerError, new { mensaje = "Error de conexión a SAP B1" });
                }
                else
                {
                    _logger.LogError("Conectado a SAP B1 exitosamente");
                    ...
                    _logger.LogInformation("Desconectado de SAP B1");
                }
            }
